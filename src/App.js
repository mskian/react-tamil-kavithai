...
getQuote() {
  toast.dark('🦄 Kavithai Updated', {
    ...
    axios.get("https://app.santhoshveer.com/san.php")
      .then(res=>{
...
getNewQuote = () => {
  ...
  render() {
    return (
        ...
        {this.state.persons.map(person=><p key = {person.id} dangerouslySetInnerHTML={{__html: person.content}}></p>)}
        </div>
        }
        <div className="buttons is-centered">
        <button className="button is-warning read-random" onClick={this.getNewQuote} disabled={this.state.isLoading}>{this.state.isLoading ? "Loading..." : "🔄 Random kavithai"}</button>
        <ToastContainer />
        <CopyToClipboard text={this.state.persons.map(person => (person.content).replace(/<br \/>/g, "\n"))} onCopy={() => this.setState({copied: true})}>
        <button className="button is-link read-random" onClick={this.Copykavithai} disabled={this.state.isLoading}>{this.state.isLoading ? "Loading..." : "📝 Copy kavithai"}</button>
        </CopyToClipboard>
        </div>
        <br />
        <br />
        ...
        <p style={{ color : '#9dad7f'}} className="has-text-weight-bold has-text-centered">
          ...
          <br />
          <img src="./react.png" loading="lazy" height="45" width="45" alt="Tamil SMS" />
        </p>
        </Fade>
        <br />
        <br />
